...
from ixmp4.services.base import AbstractService
from ixmp4.services.transport import DirectTransport, HttpxTransport
...
class IncompleteFastApiEndpointOptions(BaseFastApiEndpointOptions, total=False):
    ...
    response_class: type[fa.Response] | None
...
class ServiceProcedure(Generic[ServiceT, Params, ReturnT]):
    ...
    def __init__(
        ...
        if fastapi_options["path"] is None:
            kebab_func_name = self.func.__name__.replace("_", "-")
            fastapi_options["path"] = "/" + kebab_func_name
        if fastapi_options["name"] is None:
            fastapi_options["name"] = self.func.__name__
        if fastapi_options["methods"] is None or len(fastapi_options["methods"]) == 0:
            fastapi_options["methods"] = ["POST"]
        if fastapi_options["response_class"] is None:
    ...
    ) -> "ServiceProcedure[Any, Any, Any]": ...

    @overload
    def __get__(
        self, obj: AbstractService, cls: type[AbstractService] | None = None
    ) -> Callable[Params, ReturnT]: ...

    def __get__(self, obj: Any, cls: type[Any] | None = None) -> Any:
        if not isinstance(obj, AbstractService):
            return self
        else:
    ...
    def get_client(self, service: AbstractService) -> Callable[Params, ReturnT]:
...
class ServiceProcedureClient(Generic[ServiceT, Params, ReturnT]):
    ...
    def __call__(self, *args: Params.args, **kwargs: Params.kwargs) -> ReturnT:
        ...
        return cast(
            ReturnT, self.procedure.return_type_adapter.validate_python(res.json())
        )
