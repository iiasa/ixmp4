...
@all_platforms
def test_create_iamc_variable(test_mp):
...
@all_platforms
def test_iamc_variable_unique(test_mp):
...
@all_platforms
def test_iamc_variable_not_found(test_mp):
    with pytest.raises(Variable.NotFound):
        test_mp.iamc.variables.get("Variable")


@all_platforms
def test_filter_iamc_variable(test_mp):
