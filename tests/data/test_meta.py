...
@all_platforms
def test_create_get_entry(test_mp):
...
@all_platforms
def test_entry_not_found(test_mp):
...
@all_platforms
def test_tabulate_entry(test_mp):
    run = test_mp.Run("Model", "Scenario", "new")
...
@all_platforms
def test_entry_bulk_operations(test_mp):
    ...
    remove_data = entries.head(len(entries) // 2).drop(columns=["value", "id", "type"])
...
@all_platforms
def test_meta_bulk_exceptions(test_mp):
